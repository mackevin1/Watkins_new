...
## --------------------------------------------------------------------------- #
##  ApHP DataGrid version 4.2.8 (adapted for ApPHP DataGrid Wizard)            #
...
## Last changes:
## --------------------------------------------------------------------------- #
## 03.03.2011 - check for sensitivity of filtering fields
## 12.04.2011 - fixed problem with mb_StrToLower
...
Class DataGrid
{
    ...
    protected function SecurityCheck(){
        // check rid variable
        $req_rid = $this->GetVariable('rid');
        if(preg_match("/'/i", $this->rid) || preg_match('/"/i', $this->rid) || preg_match("/%27/", $this->rid) || preg_match("/%22/", $this->rid)){
            return false;
        }
        // check query string
    ...
    protected function SetCommonJavaScript(){
        ...
        echo "\n<!-- This script was generated by datagrid.class.php v.".self::$dgVersion." (http://phpbuilder.blogspot.com) -->";
        ...
        if($this->browser_name == "Firefox"){
            ...
            echo "
                for (var idx=0; idx < document.".$this->uniquePrefix."frmEditRow.length; idx++) {
                    field_name = document.".$this->uniquePrefix."frmEditRow.elements.item(idx).name;                    
                    field_type = document.".$this->uniquePrefix."frmEditRow.elements.item(idx).type;                    
                    // check file or image fields
                    if(field_type == 'file'){
                        if(document.getElementById(field_name).value != ''){
                           alert('You need to upload file or image before update! Click on Upload link.');
